...
alter table public.waitlist add column if not exists seq bigint generated by default as identity;

-- ---------------------------------------------------------------------------
-- join_waitlist — inserts a signup and returns its incremental number.
-- Runs security-definer so the app (anon key) can read back the `seq` number
-- without a SELECT policy on the table. Duplicates (same email) don't error:
-- the original seq is returned with duplicate = true, so the owner is still
-- notified of the repeat attempt instead of it being silently swallowed.
-- ---------------------------------------------------------------------------
...
begin
  insert into public.waitlist (
    name, first_name, last_name, email, whatsapp, country,
    gender, age, goal, height, weight, activity, experience, body_type,
    strategy, maintenance_calories, target_calories, protein, carbs, fat,
    timeline_weeks, bmr, tdee, bmi, source
  ) values (
    payload->>'name', payload->>'first_name', payload->>'last_name',
    payload->>'email', payload->>'whatsapp', payload->>'country',
    payload->>'gender', (payload->>'age')::int, payload->>'goal',
    (payload->>'height')::int, (payload->>'weight')::numeric,
    payload->>'activity', payload->>'experience', payload->>'body_type',
    payload->>'strategy', (payload->>'maintenance_calories')::int,
    (payload->>'target_calories')::int, (payload->>'protein')::int,
    (payload->>'carbs')::int, (payload->>'fat')::int,
    (payload->>'timeline_weeks')::int, (payload->>'bmr')::int,
    (payload->>'tdee')::int, (payload->>'bmi')::numeric,
    coalesce(payload->>'source', 'section')
  )
  returning seq into v_seq;

  return jsonb_build_object('seq', v_seq, 'duplicate', false);
exception when unique_violation then
  select seq into v_seq from public.waitlist where email = payload->>'email';
  return jsonb_build_object('seq', v_seq, 'duplicate', true);
end;
$$;
